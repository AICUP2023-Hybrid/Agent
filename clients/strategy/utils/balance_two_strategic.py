...
def balance_troops_between_two_strategics(gdata: GameData, src: Node, dst: Node, return_danger=False):
    troop_cnt = src.number_of_troops

    src.save_version()
    dst.save_version()
    min_danger, move_troops = np.Inf, 0
    for fort_node in ([src, dst] if not gdata.done_fort else [None]):
